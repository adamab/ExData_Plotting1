...
## Join Date and Time and assign to variable
datetime <- paste(data$Date, data$Time) %>%
        strptime(format = "%Y-%m-%d %H:%M:%S")

## Create 4 line graphs and send to PNG file
png(filename = "plot4.png") ## Open the PNG file device
par("mfrow" = c(2,2)) ## Set graph to display 4 at a time, fill by row
...
## Plot line graph for Sub metering 1 through 3
...
legend("topright", legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3")
       , lwd = 1, col = c("black", "red", "blue"), bty = "n") ## Add the legend to the graph

## Plot line graph for Global_reactive_power
